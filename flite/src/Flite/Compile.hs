...
import           Data.List            (elemIndex, findIndex, findIndices, elemIndices,
                                       partition, sortBy)
...
apReorder n s bs apps = case groupApps apps of
  [] -> []
  (as:rest) -> let nodeps = sortBy spineCmp $
                            sortBy worstSpan as
                   here = take 1 nodeps
...
checkApSpans n aspan t@(f, _, _, _, s, as)
...
trDefn n m nregs aspan p (f, args, xs)
  ...
  | otherwise
  = (f, length args, spark, luts, pushs', apps')
  where
    ...
    spark = let stricts = [ i | (('!':arg), i) <- zip args [0..]]
            in if length stricts >= 2 then Just (stricts !! 1) else Nothing
            -- TODO A better usage analysis than just taking the last arg please
...
tr p (f, args, spine, body) (Var v) =
  -- trace (unwords [f, v, show spine, show body, "\n", show varInds, show parInds, show varUses]) $
  ...
  where
    {-
    varInds = map (elemIndices (Var v)) (spine : map snd body)
    parInds = [[]] -- map (map pred . elemIndices (Fun "par")) (spine : map snd body)
    varUses = zipWith (\vs ps -> filter (`notElem` ps) vs ) varInds parInds
    shared = length (concat varUses) > 1
    -}
...
flagFuns i = map flag
  where
    flag (f, pop, spark, luts, push, apps) =
      (f, pop, spark, luts, map fl push, map (mapAtoms fl) apps)
...
frag s n m (f, pop, spark, luts, push, apps)
  -- Violated template dimensions
  | length apps > n || length luts > m || length push > s
    = fr s n m (f, pop, spark, luts, push, apps)
  -- We have PRS candidates, so might need to reorder
  | any isPRIM apps = fr s n m (f, pop, spark, luts, push, apps)
  -- OK
  | otherwise = return (f, pop, spark, luts, push, apps)

fr s n m tmpl@(f, pop, spark, luts, push, apps) =
  do x <- newId
     ...
     t <- frag s n m (f, 0, Nothing, luts', push', apps')
     write (x, t)
     return (f, pop, spark, luts0, R.FUN False 0 x : push0, apps0)
  where
