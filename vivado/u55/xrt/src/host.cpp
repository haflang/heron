...
int main(int argc, char* argv[]) {
    ...
    parser.addSwitch("--mask", "-m", "PE enable mask", "0xFFFFFF");
    parser.parse(argc, argv);

    // Read settings
    std::string binaryFile = parser.value("xclbin_file");
    std::string templateFile = parser.value("template_file");
    int device_index = stoi(parser.value("device_id"), NULL, 0);
    int gc_thres = stoi(parser.value("gc_thres"), NULL, 0);
    unsigned int mask = stoi(parser.value("mask"), NULL, 0);
    ...
    krnl.write_register(A_MASK, mask);
