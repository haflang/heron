...
We only modify `accessRam` to model READ_FIRST behaviour.

Original is from
https://github.com/clash-lang/clash-compiler/commit/4a322a18e0399e0ccd0a7715b9364999aa7c7f24
-}
...
module Heron.Primitives.ReadFirstTDPModel where
...
import           Clash.Signal.Internal   (Clock (..), ClockAB (..),
                                          KnownDomain (..), Signal (..),
                                          clockTicks)
...
-- | Model configuration for TDP RAMs
