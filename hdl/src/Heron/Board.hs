...
heronMesh
  ...
  -> SNat (rows+1)
  -> SNat (slrs+1)
  -> Vec ((slrs+1)*(rows+1)) (Vec (cols+1) (Signal DomIn Bool))
  -> Signals DomIn PEIn
  -> Signals DomIn PEOut
heronMesh x y slrs mask PEIn{..} = PEOut {..}
  where
    (_, _, _, _, _peResult, _peStats) = genSuperMesh slrs mask peGo peRelease peCode peGcThreshold nullColRxs nullRowRxs nullColRxs nullRowRxs
...
type HeronIn = Signal DomIn
  ...
  , "gcThres"  ::: HeapAddr
  -- ^ GC Threshold
  , "mask"     ::: BitVector (MeshRows * Meshes * MeshCols)
  -- ^ Mask bits for enabling PEs
  )
...
topEntity clkIn rstIn _enIn ins = reg defOut $ bundle
  ...
  where
    (cWe, cAddr, cData, go, unlock, gcThres, rawmask) = unbundle $ reg defIn ins
    ...
    defIn  = (False, 0, unpack 0, False, False, 0, 0)

    mask = unconcat (SNat @MeshCols) . unbundle $ map bitToBool . bv2v <$> rawmask'
    ...
    peOut = withClockResetEnable @DomIn clkIn rstIn en $
            ...
            heronMesh meshX meshY meshZ mask peIn
...
vioHeron clk = fmap mergeTmpl . vioProbe @DomIn ins outs defOut clk
  where
    mergeTmpl (cw,ca,cdLsb,cdMsb,go,rel,gt,m) = (cw,ca, resize (cdMsb ++# cdLsb),go,rel,gt,m)
    ins  = "ret" :> "stats" :> "retVld" :> Nil
    outs = "codeWE" :> "codeAddr" :> "codeDataLsb" :> "codeDataMsb" :> "go" :> "unlock" :> "gcThres" :> "mask" :> Nil
    defOut = (False, 0 :: Index RomSize, unpack 0 :: BitVector 256, unpack 0 :: BitVector 256, False, False, 0 :: HeapAddr, 0)
