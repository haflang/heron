...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, "g:r:d:m:")) != -1) {
    switch (opt) {
      case 'g':
        gc_thres = (int) strtoul(optarg, NULL, 0);
        top->gcThres  = gc_thres;
        break;
      case 'm':
        mask = (int) strtoul(optarg, NULL, 0);
        top->mask  = mask;
        break;
      case 'r':
        limit = (int) strtoul(optarg, NULL, 0);
        break;
      case 'd':
        wait = (int) strtoul(optarg, NULL, 0);
        break;
      default:
        usage();
        return 1;
